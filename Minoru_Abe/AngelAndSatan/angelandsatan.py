...
def debut(c=TAILLE):
    ...
    image=open("angelAndSatan.svg","w")
    image.write(entete)
    return image
...
if __name__=="__main__":
    ...
    image.write(rectangle(largeur=9*cote+2*ep,hauteur=11*cote+2*ep,rx=rx,ry=ry))
    

    # rectangle interieur (bords droits)
    decalx=ep
    decaly=ep
    # en fait c'est pas un vrai rectangle, il y a des lignes à ne pas couper
    # Les deux cotés verticaux
    image.write(ligne((0*cote,0*cote),(0*cote,11*cote)))
    image.write(ligne((9*cote,0*cote),(9*cote,11*cote)))
    # Le haut
    # en fait non, il faut un tenon
    #image.write(ligne((0*cote,0*cote),(3*cote,0*cote)))
    image.write(ligne((0*cote,0*cote),(1*cote,0*cote)))
    image.write(ligne((2*cote,0*cote),(3*cote,0*cote)))
    image.write(ligne((1*cote,1*cote),(2*cote,1*cote)))
    image.write(ligne((1*cote,0*cote),(1*cote,1*cote)))
    image.write(ligne((2*cote,0*cote),(2*cote,1*cote)))

    
    #image.write(ligne((6*cote,0*cote),(9*cote,0*cote)))
    image.write(ligne((6*cote,0*cote),(7*cote,0*cote)))
    image.write(ligne((8*cote,0*cote),(9*cote,0*cote)))
    image.write(ligne((7*cote,1*cote),(8*cote,1*cote)))
    image.write(ligne((7*cote,0*cote),(7*cote,1*cote)))
    image.write(ligne((8*cote,0*cote),(8*cote,1*cote)))

    # Pièce du milieu à gauche
    image.write(ligne((0*cote,4*cote),(1*cote,4*cote)))
    image.write(ligne((2*cote,4*cote),(3*cote,4*cote)))
    image.write(ligne((1*cote,5*cote),(2*cote,5*cote)))
    image.write(ligne((1*cote,4*cote),(1*cote,5*cote)))
    image.write(ligne((2*cote,4*cote),(2*cote,5*cote)))

    # Pièce de la colonne centrale en haut
    image.write(ligne((3*cote,3*cote),(6*cote,3*cote)))

    # Pantalons
    image.write(ligne((1.5*cote,9*cote),(1.5*cote,11*cote), couleur="red"))
    image.write(ligne((7.5*cote,9*cote),(7.5*cote,11*cote), couleur="red"))

    
    # Le bas
    image.write(ligne((0*cote,11*cote),(3*cote,11*cote)))
    image.write(ligne((6*cote,11*cote),(9*cote,11*cote)))
    
    #image.write(rectangle(largeur=9*cote,hauteur=11*cote,rx=0,ry=0,couleur="blue"))
    #Verticales
    image.write(ligne((3*cote,0*cote),(3*cote,11*cote)))
    image.write(ligne((6*cote,0*cote),(6*cote,11*cote)))
    #Horizontales
    image.write(ligne((0*cote,4*cote),(3*cote,4*cote)))
    image.write(ligne((6*cote,4*cote),(9*cote,4*cote)))
    image.write(ligne((0*cote,7*cote),(3*cote,7*cote)))
    image.write(ligne((6*cote,7*cote),(9*cote,7*cote)))
    # Centre
    image.write(ligne((3*cote,1*cote),(6*cote,1*cote)))
    image.write(ligne((3*cote,10*cote),(6*cote,10*cote)))
